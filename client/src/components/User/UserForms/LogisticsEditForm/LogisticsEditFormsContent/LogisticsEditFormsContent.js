...
const LogisticsEditFormsContent = () => {
	...
	useEffect(() => {
		...
		axios.get("http://localhost:8080/api/logisticsrecords")
			.then(res => {
				setID(res.data[0])
				// console.log("this is a get by id reduest",res.data[0]._id)
			})
			.catch(err => console.log(err))
	}, [])

	console.log("Data fron id state",id)
	...
	const formik = useFormik({
		...
		onSubmit: (values, { resetForm }) => {
			try {
				axios
					.post(`http://localhost:8080/api/logisticsrecords/${id}`, values)
				...
				>
					<div className="logistics-quotation-small-inputs">
						<div className="logistics-quotation-small-input-group">
							...
							<input
								type="text"
								placeholder={id ? id.fullnames : "enter fullnames"}
								name="fullnames"
					...
					<div className="logistics-quotation-small-inputs">
						...
						<div className="logistics-quotation-small-input-group">
							...
							<input
								...
								required
							/>
							{formik.touched.timeevents && formik.errors.timeevents ? (
								<div className="error">{formik.errors.timeevents}</div>
							) : null}
						</div>
						<div className="logistics-quotation-small-input-group">
							<label>Unit</label>
...
export default LogisticsEditFormsContent;
